...
+
When you run this command, it will create a VPC Peering connection from your environment to the VPC of your Hazelcast Dedicated cluster, then it will create the necessary routes for the VPC Peering connection. Lastly, it will accept this connection request from the Hazelcast Cloud side.
+
...
You can get these properties from the link:{url-cloud-api}[GraphQL API] or the link:{url-github-go-sdk}[Hazelcast Cloud SDK] as the following command.
...
WARNING: The CIDR of your Hazelcast Dedicated cluster and the CIDR of your VPC *should not overlap,* otherwise you will not see a connection in this list.
