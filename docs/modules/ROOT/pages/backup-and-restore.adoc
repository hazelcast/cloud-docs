...
When you have at least one backup, you can use it to bootstrap a new cluster. The new cluster is created using the current Hazelcast version. To see the Hazelcast version in use, either look at the xref:hazelcast:release-notes:releases.adoc[release notes] or go to the dashboard of the new cluster and view the *Cluster Details*.
