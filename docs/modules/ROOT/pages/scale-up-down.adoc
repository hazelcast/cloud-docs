...
Dedicated clusters scale horizontally, which means that you can adjust the resources available to your application, such as memory, by adding members (scaling out) or removing members (scaling in). Each dedicated cluster is a network of a minimum of three instances or _members_, with a fixed set of resources allocated to each one.
