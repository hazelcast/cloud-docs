= Dedicated Clusters
:description: {hazelcast-cloud} Dedicated is a fully-managed cloud service that allows you to maintain control over the hardware, resources, and deployment options that are available to your cluster. Dedicated means you get specific servers (cloud instances) all to yourself. 
:page-dedicated: true
...
[discrete] 
==== Service Design
- Specify the memory capacity and number of nodes for all provisioned clusters. You decide when to scale your clusters if they need more resources.  
- Deploy clusters to specific regions or Cloud providers as part of your organization's Cloud strategy.  To reduce network latency, you can deploy clusters in regions that are closest to the location of your users.
