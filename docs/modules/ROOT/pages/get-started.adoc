...
+
.Windows
At the moment, Hazelcast does not support any Windows package managers. The easiest way to use the Hazelcast CLI on Windows is to install the Hazelcast Enterprise Docker image. The Docker container comes with the `hz-cli` client. See xref:hazelcast:getting-started:install-enterprise.adoc[].

. Check that the Hazelcast CLI client is installed.
+
[source,shell]
----
hz-cli -V
----
+
You should see your installed version of the Hazelcast CLI client.
...
. In the *Quick Connection Guide*, go to step 2 and run the curl command to import the configuration of your {hazelcast-cloud} cluster.

. Now execute one of the following commands to connect the Hazelcast CLC to your cluster.
** For {hazelcast-cloud} Trial:
+
```bash
clc -c viridian
```

** For other environments, run the following command replacing the placeholder `$CLUSTER_ID`. You can find the cluster ID on the dashboard of your cluster under *Cluster Details*.
+
```bash
clc -c de-$CLUSTER_ID
```
...
[source,bash]
----
clc sql -c de-$CLUSTER_ID "CREATE OR REPLACE MAPPING cities (
__key INT,
country VARCHAR,
city VARCHAR,
population INT)
...
[source,sql]
----
clc sql -c de-$CLUSTER_ID "INSERT INTO cities VALUES
(1, 'United Kingdom','London', 9540576),
(2, 'United Kingdom','Manchester', 2770434),
(3, 'United States', 'New York', 19223191),
(4, 'United States', 'Los Angeles', 3985520),
...
NOTE: The functionality in the following steps is not currently available for {hazelcast-cloud} Trial.
...
- link:https://github.com/hazelcast/hazelcast-packaging[SQL (CLI) client]
