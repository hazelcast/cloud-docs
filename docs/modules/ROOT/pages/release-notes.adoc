...
- Azure Storage Account is not deleted during cluster deletion
- Field label not matching the placeholder text
- Wan Replication fails on Azure clusters
...
- Enabled the xref:imdg:clusters:deploying-code-on-member.adoc[User code deployment feature] for Hazelcast Cloud. You can now run the executor service, entry processor and queries with custom objects.
...
- Autoscaling: Introduced xref:manage-the-cluster.adoc[automatic scaling] (up or down) of your cluster depending on the memory utilization.
